...
var nodes1 = d3.range(10).map(function(i) {
    return {
        index: i,
...
var nodes2 = d3.range(10).map(function(i) {
    return {
        index: i,
        id: ~~(Math.random() * foci.length),
        r: Math.random() * 10 + 5
...
for(var i = 0; i < nodes.length; i ++){
    simulations[i] = d3.forceSimulation(nodes[i])
...
function ticked() {
    ...
    for(k = 0; k < nodes.length; k ++){
        for(var i = 0; i < nodes[k].length; i ++){
            var d = nodes[k][i];
            ...
            var x = (d.x - (width / 4 - cent[0])),
                y = (d.y - (height / 4 - cent[1])),
                inter = false;
